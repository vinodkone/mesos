...
case $BUILDTOOL in
  autotools)
    ...
    ;;
  cmake)
    # CMake does not have distcheck support out of the box, we should add it as
    # stated in [MESOS-3108], but we can do check to pass basic tests.
    append_dockerfile "CMD ./bootstrap && mkdir ./build && cd ./build/ && cmake ../ $CONFIGURATION && make -j8 check"
    ;;
  *)
